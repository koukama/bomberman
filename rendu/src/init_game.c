...
** Last update Thu Feb 14 12:54:55 2019 REBOUL Simon
*/
...
SDL_Renderer	*init_SDL(t_start_screen *start_screen)
{
  ...
  start_screen->font = TTF_OpenFont("./fonts/Square.ttf", 75);

  start_screen->fontColor = black;
  start_screen->fontColorBg = white;
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER) < 0)
    my_putstr("Can't init SDL !");
  ...
  window = SDL_CreateWindow("Bomberman",
			    ...
			    SCREEN_W, SCREEN_H,
			    SDL_WINDOW_OPENGL + 
			    SDL_WINDOW_RESIZABLE+ SDL_RENDERER_SOFTWARE);
...
void	init_startup(t_start_screen *start_screen)
{
  ...
  start_screen->multiBgSprite = IMG_Load("./images/multibg.png");
  ...
  start_screen->winBgSprite = IMG_Load("./images/victory.png");
  start_screen->winBgTexture = SDL_CreateTextureFromSurface(start_screen->screen, start_screen->winBgSprite);
  start_screen->winButtonSprite = IMG_Load("./images/button_win.png");
  start_screen->winButtonTexture = SDL_CreateTextureFromSurface(start_screen->screen, start_screen->winButtonSprite);
  start_screen->yesButtonSprite = IMG_Load("./images/button_yes.png");
  start_screen->yesButtonTexture = SDL_CreateTextureFromSurface(start_screen->screen, start_screen->yesButtonSprite);
  start_screen->noButtonSprite = IMG_Load("./images/button_no.png");
  start_screen->noButtonTexture = SDL_CreateTextureFromSurface(start_screen->screen, start_screen->noButtonSprite);
  ...
  start_screen->winButtonPos.x = SCREEN_W / 2 + 100;
  start_screen->winButtonPos.y = SCREEN_H / 2 + 175;
  start_screen->winButtonPos.w = 204;
  start_screen->winButtonPos.h = 43;

  start_screen->winPlayerPos.x = SCREEN_W / 2 - 80;
  start_screen->winPlayerPos.y = SCREEN_H / 2 + 50;
  start_screen->winPlayerPos.w = 150;
  start_screen->winPlayerPos.h = 150;
...
void	init_game(t_game *game, t_start_screen *start_screen)
{
  ...
  game->playersPtr = (t_players*)malloc(sizeof(t_player) * 5 + 1);
  game->playersPtr->nb_players = 0;
  game->socket = create_bind_listen();
  ret = pthread_create(&new_conn, NULL, accept_new_client, (void*)game);
  if (ret)
    printf("Cannot create this fucking thread\n\n");
...
t_player	*init_player(t_game *game, int socket)
{
  ...
  for (i = 0; game->players[i] != NULL; i++)
    {
      if (i == 0)
	game->playersPtr->first = game->players[i];
      if (game->players[i + 1] != NULL)
	{
	  game->playersPtr->last = game->players[i + 1];
	  game->players[i]->next = game->players[i + 1];
	  game->players[i + 1]->prev = game->players[i];
	}
    }
  game->playersPtr->nb_players++;
  game->players[i] = (t_player *)malloc(sizeof(t_player) + 1);
  game->players[i]->id = i + 1;
  my_putstr("Player ");
  my_put_nbr(game->players[i]->id);
  my_putstr(" created\n");
  game->players[i]->cli_socket = socket;
